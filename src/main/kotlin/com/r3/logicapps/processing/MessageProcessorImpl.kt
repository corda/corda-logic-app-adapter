...
open class MessageProcessorImpl(
    ...
    override fun invoke(message: BusRequest): BusResponse = when (message) {
        is BusRequest.InvokeFlowWithoutInputStates -> processInvocationMessage(message.requestId, message, true)
        is BusRequest.InvokeFlowWithInputStates -> processInvocationMessage(message.requestId, message, false)
        is BusRequest.QueryFlowState -> {
            TODO("Handler for QueryFlowState not implemented")
        }
    }

    private fun processInvocationMessage(requestId: String, invocable: Invocable, isNew: Boolean): BusResponse {
        return try {
            ...
            BusResponse.FlowOutput(
                ...
                requestId = requestId,
                linearId = result.linearId ?: error("Unable to derive linear ID after flow invocation"),
